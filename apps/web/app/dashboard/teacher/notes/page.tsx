...
export default function TeacherNotesPage() {
  ...
  // Deduplicate divisions from assignments
  const divisions = assignments.reduce<{ id: number; label: string }[]>((acc, a) => {
    if (!acc.find((d) => d.id === a.division_id)) {
      acc.push({ id: a.division_id, label: a.division_name });
    }
    return acc;
  }, []);
  ...
  useEffect(() => {
    fetchNotes();
    api.get<Assignment[]>("/attendance/my-assignments").then(({ data }) => setAssignments(data)).catch(() => {});
  }, []);
