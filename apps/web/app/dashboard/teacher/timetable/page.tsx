  ...
  return (
    <div className="rounded-md border border-gray-200 bg-white px-3 py-2 shadow-sm">
      ...
      <div className="mt-1 flex flex-wrap gap-3 text-xs text-gray-500">
        ...
        <span>Yr {slot.year} Div {slot.division}</span>
        <span>{slot.room}</span>
      </div>
    </div>
  );
...
export default function TeacherTimetablePage() {
  ...
  const fetchData = useCallback(() => {
    if (!user) return;
    setLoading(true);
    const todayReq = api.get("/timetable/my-today");
    const weekReq = api.get("/timetable/my-week");
    Promise.all([todayReq, weekReq])
      .then(([t, w]) => {
        setTodayData(t.data);
        setWeekData(w.data);
      })
        ...
        <div className="space-y-4">
          ...
          {isHolidayOrExam(todayData.schedule_events) ? (
            ...
            <div className="space-y-3">
              <p className="text-sm font-medium text-gray-700">{todayData.slots.length} class{todayData.slots.length > 1 ? "es" : ""} today</p>
