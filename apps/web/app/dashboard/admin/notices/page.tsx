...
export default function AdminNoticesPage() {
  ...
  useEffect(() => {
    fetchNotices();
    api.get<Dept[]>("/admin/departments").then(({ data }) => setDepts(data)).catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div>
            ...
            >
              <option value="">Institution Level (All Branches)</option>
              {depts.map((d) => (
                <option key={d.id} value={d.id}>Branch Specific — {d.name}</option>
              ))}
            </select>
          </div>
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Megaphone className="h-5 w-5" /> All Notices</CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            <Spinner />
          ) : notices.length === 0 ? (
